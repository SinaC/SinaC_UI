...
if not C.general.dbmreskin then return end
		...
		for bar in self:GetBarIterator() do
			if not bar.injected then
					...
					if not (icon1.overlay) then
						...
						icon1.overlay:Point("BOTTOMRIGHT", tbar, "BOTTOMLEFT", -buttonsize/4, -2)
					...
					if not (icon2.overlay) then
						...
						icon2.overlay:Point("BOTTOMLEFT", tbar, "BOTTOMRIGHT", buttonsize/4, -2)
...
local Init = function()
	...
	if IsAddOnLoaded("Tukui_DBM") then return end
	LoadSkin()
