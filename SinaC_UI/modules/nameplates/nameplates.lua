...
local function UpdateAuraIcon(button, unit, index, filter)
	...
	button.cd:SetScript("OnUpdate", function(self)
		...
		button.cd.timer.text:SetFont(FONT, 8, FONTFLAG)
...
--We need to reset everything when a nameplate it hidden, this is so theres no left over data when a nameplate gets reshown for a different mob.
...
--print("COLORIZE 1")
			frame.hp:SetStatusBarColor(unpack(T.UnitColor.class[class]))
			return
		end
	end
	frame.isTagged = nil
...
local function UpdateObjects(frame)
	...
	if C["nameplate"].showlevel == true then
		...
		if frame.hp.boss:IsShown() then
			...
			frame.hp.level:Show()
		--elseif not elite and level == mylevel then -- Display mob with same level
		--	frame.hp.level:Hide()
		else
	...
	if C["nameplate"].enhancethreat ~= true then
		...
		if not frame.threat:IsShown() then
			...
			if g + b == 0 then
				--Have Threat
				if Role == "Tank" then
--print("1:"..tostring(goodR).."  "..tostring(goodG).."  "..tostring(goodR))
					frame.hp:SetStatusBarColor(goodR, goodG, goodB)
					frame.hp.hpbg:SetTexture(goodR, goodG, goodB, 0.25)
					frame.threatStatus = "GOOD"
				else
...
function NamePlates:COMBAT_LOG_EVENT_UNFILTERED(_, event, _, sourceGUID, _, _, _, destGUID, _, _, _, spellID)
